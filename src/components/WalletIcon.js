...
const WalletIcon = ({ fresh_creator_wallet, creator, size = "normal" }) => {
  ...
  const getTitle = () => {
    if (!creator) return "";
    if (fresh_creator_wallet === true) return "Fresh wallet detected - Click to view transactions on Solscan";
    if (fresh_creator_wallet === false)
      return "Wallet has transaction history - Click to view details on Solscan";
    return "Click to view wallet on Solscan";
  };
